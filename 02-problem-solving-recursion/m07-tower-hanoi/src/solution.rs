...
{
    // Recursively move num_to_move - 1 disks
    // from from_post to temp_post
    // using to_post as temporary storage.
    if num_to_move > 1 {
        move_disks(posts, num_to_move - 1, from_post, temp_post, to_post);
    }

    // Move the last disk from from_post to to_post.
    move_disk(posts, from_post, to_post);
    draw_posts(posts);

    // Recursively move num_to_move - 1 disks
    // from temp_post to to_post
    // using from_post as temporary storage.
    if num_to_move > 1 {
